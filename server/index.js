...
if (!process.env.MONGODB_URI) {
  console.error("❌ MONGODB_URI environment variable is not set. Set it in AWS App Runner → Configuration → Environment variables.");
  process.exit(1);
}
...
const { TeamMember, Partner, NarrSnapshot, Opportunity, UploadLog, Champion, Workstream, Activity, User, Config, MarketingEvent } = require("./models");
...
mongoose.connect(process.env.MONGODB_URI)
  .then(async () => {
    ...
    const DEFAULTS = [
      { key:"partners",        label:"Partners",             values:["Infosys","EY","Wipro","Accenture","Tech Mahindra","TCS","Hexaware","Capgemini","Cognizant","LTIM"] },
      { key:"activityOwners",  label:"Activity Owners",      values:["Deepak M","Sampat B","Iman R","Meena M","Shivani T","Sneha D","Rashmi N","Vikas S","Ashutosh B","Prasad P","Haim R"] },
      { key:"championOwners",  label:"Champion Contact Owners", values:["Sneha D","Rashmi N","Vikas S","Ashutosh B","Prasad P","Haim R","Will Winn","Stefano S","Piyush A","Vinai","Harshada","Cameron"] },
      { key:"regions",         label:"Regions",              values:["APAC","AMER","LATAM","EU","ME","Africa"] },
      { key:"priorities",      label:"Priorities",           values:["P1","P2","P3"] },
      { key:"activityCategories", label:"Activity Categories", values:["Workstream","Champion Building","Opportunity","PDM","Event Planning"] },
      { key:"activityStatuses",label:"Activity Statuses",    values:["Open","In Progress","Done","Blocked"] },
      { key:"impacts",         label:"Impact Levels",        values:["High","Medium","Low"] },
      { key:"workstreamStatuses", label:"Workstream Statuses", values:["Active","Inactive","On Hold"] },
      { key:"certifications",  label:"Certification Types",  values:["SI Associate","SI Architect","App Dev"] },
      { key:"championStatuses",label:"Champion Statuses",    values:["champion","building","not_yet"] },
      { key:"teamMembers",     label:"Team Members",         values:["Iman Roy","Deepak Mirchandani","Meena M","Shivani Tripathy"] },
    ];
...
app.get("/api/dashboard", async (req, res) => {
  try {
    ...
    if (req.query.week) {
      ...
      if (!latest) return res.json({ team: [], availableWeeks: [], teamTarget: TEAM_TARGET });
    ...
    const snapshotMap = {};
    snapshots.forEach(s => { snapshotMap[s.partner.toString()] = s; });
    ...
    }).populate("partner").populate("championId").populate("workstreamId");
    ...
    for (const opp of allApprovedForRegion) {
      ...
      if (!regionalNarrMap[ownerName]) regionalNarrMap[ownerName] = { AMER: 0, EMEA: 0, APAC: 0, LATAM: 0, other: 0 };
      const region = opp.region || "";
      const key = ["AMER","EMEA","APAC","LATAM"].includes(region) ? region : "other";
    ...
    function serializeOpp(opp, extra = {}) {
      return {
        ...
        attributionCreatedDate: opp.attributionCreatedDate ? opp.attributionCreatedDate.toISOString().split("T")[0] : "",
        oppCreatedDate:         opp.oppCreatedDate ? opp.oppCreatedDate.toISOString().split("T")[0] : "",
    ...
    for (const opp of allApprovedOpps) {
      ...
      memberMap[ownerName].quarterlyOpportunities.push({
        _id: opp._id, id: opp.oppId,
        partner: opp.partner?.name || "", title: opp.title,
        value: opp.value, stage: opp.stage, type: opp.motionType,
        status: opp.status || "", fiscalPeriod: opp.fiscalPeriod || "",
        close: opp.closeDate ? opp.closeDate.toISOString().split("T")[0] : "",
      });
    }
    ...
    const ORDER = ["Iman Roy","Deepak Mirchandani","Meena M","Shivani Tripathy"];
    const team = Object.values(memberMap).sort((a,b) => {
      const ai = ORDER.indexOf(a.name), bi = ORDER.indexOf(b.name);
      return ai===-1&&bi===-1?a.name.localeCompare(b.name):ai===-1?1:bi===-1?-1:ai-bi;
    });
...
app.get("/api/history/:partnerName", async (req, res) => {
  try {
    ...
    const snapshots = await NarrSnapshot.find({ partner: partner._id })
      .sort({ weekDate: 1 });
...
app.patch("/api/opportunities/:id", async (req, res) => {
  try {
    ...
    if (opp?.workstreamId) {
      const linked = await Opportunity.find({ workstreamId: opp.workstreamId, status: { $regex: /^approved$/i } });
      const narrGen  = linked.filter(o => o.stage === "Closed").reduce((s,o) => s + o.value, 0);
      const pipeLine = linked.filter(o => o.stage !== "Closed").reduce((s,o) => s + o.value, 0);
...
app.get("/api/opportunities/list", async (req, res) => {
  try {
    const opps = await Opportunity.find({ status: { $regex: /^approved$/i } })
      .populate("partner").populate("championId").populate("workstreamId")
      .sort({ createdDate: -1 }).limit(500);
    res.json(opps.map(o => ({
      _id: o._id, id: o.oppId, title: o.title,
      partner: o.partner?.name || "", value: o.value, stage: o.stage,
      championId: o.championId?._id || null, championName: o.championId?.contactName || "",
      workstreamId: o.workstreamId?._id || null, workstreamName: o.workstreamId?.workstream || "",
    })));
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post("/api/champions/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    if (req.file.originalname.match(/\.xlsx?$/i)) {
      return res.status(400).json({ error: "Please export your sheet as CSV before uploading" });
    }
    ...
    for (const row of rows) {
      ...
      await Workstream.findOneAndUpdate(
        { partnerName, workstream },
        {
          ...
          opportunitiesClosed:  parseInt(row["Opporunities ( Closed )"] || "0") || 0,
          accounts:             row["Accounts"] || "",
          pipelineNarr:         parseFloat(row["Pipeline NARR "] || row["Pipeline NARR"] || "0") || 0,
          opportunitiesPipe:    parseInt(row["Opportunities in teh pipe"] || "0") || 0,
...
app.post("/api/cleanup/partners", async (req, res) => {
  ...
  try {
    ...
    for (const partner of partners) {
      ...
      await NarrSnapshot.updateMany({ partner: partner._id }, { $set: { partner: canonicalDoc._id } });
...
app.get("/api/champions", async (req, res) => {
  try {
    ...
    const data = await Champion.find(filter).sort({ partner:1, contactName:1 });
    res.json(data);
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.put("/api/champions/:id", async (req, res) => {
  try {
    const doc = await Champion.findByIdAndUpdate(req.params.id, req.body, { new:true });
    res.json(doc);
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post("/api/champions/:id/comments", async (req, res) => {
  try {
    ...
    const doc = await Champion.findByIdAndUpdate(
      req.params.id,
      { $push: { comments: { text: text.trim(), addedBy: addedBy||"", createdAt: new Date() } } },
      { new: true }
    );
    res.json(doc);
  } catch(err) { res.status(500).json({ error: err.message }); }
...
app.get("/api/workstreams", async (req, res) => {
  try {
    ...
    const data = await Workstream.find(filter).sort({ partnerName:1, workstream:1 });
    res.json(data);
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.put("/api/workstreams/:id", async (req, res) => {
  try {
    const doc = await Workstream.findByIdAndUpdate(req.params.id, req.body, { new:true });
    res.json(doc);
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post("/api/cleanup/blocked-partners", async (req, res) => {
  const BLOCKED_PATTERNS = [
    /ecloudvalley/i, /it-valley/i, /leyun/i, /v-valley/i,
    /mckinsey/i, /nanjing\s*webeye/i, /webeye/i,
  ];

  try {
    let deletedPartners = 0, deletedSnaps = 0, deletedOpps = 0;
    ...
    const allOpps = await Opportunity.find({ rawPartnerName: { $exists: true, $ne: "" } });
    for (const opp of allOpps) {
      if (!BLOCKED_PATTERNS.some(re => re.test(opp.rawPartnerName || ""))) continue;
      await Opportunity.deleteOne({ _id: opp._id });
      deletedOpps++;
      console.log(`🗑 Deleted blocked opp by rawPartnerName: "${opp.rawPartnerName}" (${opp.title})`);
    }

    res.json({ success: true, deletedPartners, deletedSnaps, deletedOpps,
      message: `Removed ${deletedPartners} partner(s), ${deletedSnaps} snapshots, ${deletedOpps} opportunities` });
  } catch(err) { res.status(500).json({ error: err.message }); }
});

app.post("/api/cleanup/ecloudvalley", (_req, res) => res.redirect(307, "/api/cleanup/blocked-partners"));
...
app.put("/api/activities/:id", async (req, res) => {
  try {
    ...
    const doc = await Activity.findByIdAndUpdate(req.params.id, body, { new: true });
    res.json(doc);
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post("/api/users", authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const user = await User.create({ username, password: hash, role: role || "Viewer" });
...
app.get("/api/config", async (req, res) => {
  try {
    ...
    const map = {};
    configs.forEach(c => { map[c.key] = { label: c.label, values: c.values }; });
    res.json(map);
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/export/activities", authMiddleware, async (req, res) => {
  try {
    ...
    const data = await Activity.find(filter).sort({ activityDate: -1 });
    const headers = ["Partner","Custom Partner","Owner","Region","Priority","Status","Category","Impact","Revenue Impact","Activity Date","Updated Date","Department","Regional Partner Manager","Description","Doc Links"];
    const rows = data.map(a => [
      a.partner, a.partnerCustom||"", a.owner, a.region, a.priority, a.status, a.category,
      a.impact, a.revenueImpact||"",
      a.activityDate ? new Date(a.activityDate).toLocaleDateString("en-GB") : "",
      a.updateDate   ? new Date(a.updateDate).toLocaleDateString("en-GB")   : "",
      a.department, a.regionalPartnerManager, a.description,
...
app.get("/api/export/champions", authMiddleware, async (req, res) => {
  try {
    ...
    if (req.query.partner) filter.partner = req.query.partner;
    const data = await Champion.find(filter).sort({ partner: 1, contactName: 1 });
    const headers = ["Partner","Contact Name","Title","Department","Manager Name","Email","Phone","Contact Owner","Champion Status","Certifications"];
    const rows = data.map(c => [
      c.partner, c.contactName, c.title, c.department, c.managerName,
      c.email, c.phone, c.contactOwner, c.championStatus||"",
...
app.get("/api/export/workstreams", authMiddleware, async (req, res) => {
  try {
    ...
    const headers = ["Partner","Workstream","Status","DRI","Executive Sponsor","Scalable","SMART Goals","Joint Value Proposition","Ideal Customer Profile","Related Joint Assets","NARR Generated","Closed Opps","Accounts","Pipeline NARR","Pipeline Opps","Pipeline Accounts"];
    const rows = data.map(w => [
      w.partnerName, w.workstream, w.status, w.dri, w.executiveSponsor, w.scalableReplicable,
      w.smartGoals, w.jointValueProp, w.idealCustomerProfile, w.relatedJointAssets,
      w.narrGenerated, w.opportunitiesClosed, w.accounts,
      w.pipelineNarr, w.opportunitiesPipe, w.pipelineAccounts,
    ]);
    res.setHeader("Content-Type","text/csv");
    res.setHeader("Content-Disposition","attachment; filename=workstreams.csv");
    res.send(toCSV(headers, rows));
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/export/opportunities", authMiddleware, async (req, res) => {
  try {
    ...
    if (req.query.stage)   filter.stage   = req.query.stage;
    if (req.query.partner) { const p = await Partner.findOne({ name: req.query.partner }); if (p) filter.partner = p._id; }
    const data = await Opportunity.find(filter).populate("partner").sort({ createdDate: -1 });
    const headers = ["Opp ID","Title","Partner","Stage","Status","Motion Type","Value","Fiscal Period","Close Date","Attribution Created","Opp Created","Champion","Workstream","Source"];
    const rows = data.map(o => [
      o.oppId, o.title, o.partner?.name||"", o.stage, o.status, o.motionType, o.value,
      o.fiscalPeriod||"",
      o.closeDate               ? new Date(o.closeDate).toLocaleDateString("en-GB")               : "",
      o.attributionCreatedDate  ? new Date(o.attributionCreatedDate).toLocaleDateString("en-GB")  : "",
      o.oppCreatedDate          ? new Date(o.oppCreatedDate).toLocaleDateString("en-GB")          : "",
      "", "", o.source||"",
    ]);
    res.setHeader("Content-Type","text/csv");
...
app.get("/api/export/narr", authMiddleware, async (req, res) => {
  try {
    const snapshots = await NarrSnapshot.find().populate("partner").sort({ weekDate: -1 });
    const headers = ["Partner","Week Date","NARR","Closed NARR","Pipeline NARR","Strategic","Non-Strategic","Prev NARR","Prev Closed NARR"];
    const rows = snapshots.map(s => [
      s.partner?.name||"", s.weekDate ? new Date(s.weekDate).toLocaleDateString("en-GB") : "",
      s.narr, s.closedNarr, s.pipelineNarr, s.strategic, s.nonStrategic, s.prevNarr, s.prevClosedNarr,
    ]);
    res.setHeader("Content-Type","text/csv");
    res.setHeader("Content-Disposition","attachment; filename=narr_snapshots.csv");
    res.send(toCSV(headers, rows));
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.put("/api/events/:id", async (req, res) => {
  try {
    const doc = await MarketingEvent.findByIdAndUpdate(req.params.id, req.body, { new: true });
    res.json(doc);
  } catch(err) { res.status(500).json({ error: err.message }); }
});
